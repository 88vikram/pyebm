...
def Control(DataIn,MethodOptions=False,VerboseOptions=False,Factors=['Age','Sex','ICV']):
        
    from collections import namedtuple
    import pandas as pd
    ## Default Options for the function call
    DMO = namedtuple('DefaultMethodOptions',' MixtureModel Bootstrap Staging NStartpoints Niterations N_MCMC')
    ...
    t0 = time.time()
    pdData_all,UniqueSubjIDs = util.pdReadData(DataIn,flag_JointFit=0)
    ...
    for i in range(len(data_AD_raw_list)): ## For each bootstrapped iteration
        ...
        mix = np.copy(params_opt[:,4,0]); mix[:]=0.5
        params_opt[:,4,0]=0.5;
        pi0,event_centers=ca.MCMC(Data_all[:,:,0],params_opt,DMO.N_MCMC,mix,DMO.NStartpoints,DMO.Niterations);    
    ...
    if len(data_AD_raw_list)>1:
        ...
        (pi0_mean,bootstrap_event_centers) = wma.weighted_mallows.fitMallows(num_events,pi0_all,wts_all,[]);   
    else:
        pi0_mean=pi0_all[0]
    ## Visualize Results
    if DVO.Ordering==1:
        util.VisualizeOrdering(BiomarkersList, DVO.SaveFile, pi0_all,pi0_mean,DVO.PlotOrder);
    ...
    return pi0_mean,pi0_all,params_opt_all,BiomarkersList,event_centers_all
