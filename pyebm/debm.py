...
def fit(DataIn,MethodOptions=False,VerboseOptions=False,Factors=['Age','Sex','ICV'],Labels=['CN','MCI','AD'],DataTest=[],Groups=[]):
    ...
    for i in range(len(data_AD_raw_list)): ## For each bootstrapped iteration
        ...
        pi0,event_centers=cu.find_central_ordering(Data_all,p_yes,params_opt,Groups,GroupValues,DMO,'debm')
    ...
    cu.show_outputs(Data_all, Data_test_all, pdData_all, Labels, pdDataTest_all, subj_stages, subj_stages_test, params_opt,\
                 evn_full, evn, BiomarkersList,pi0_all,pi0_mean,DVO,Groups,GroupValues,'debm')                                  
