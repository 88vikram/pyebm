...
MO1.Bootstrap=0; MO1.MixtureModel='vv2'; MO1.PatientStaging=['exp','p']

MO2 = namedtuple('MethodOptions','MixtureModel Bootstrap PatientStaging')
MO2.Bootstrap=0; MO2.MixtureModel='vv2'; MO2.PatientStaging=['ml','l']

VO = namedtuple('VerboseOptions','Distributions PatientStaging')
VO.PatientStaging=0; VO.Distributions=0; 
import pandas as pd
D=pd.read_csv('ADNI_7.csv'); Y = D['Diagnosis'].copy(); Y[Y=='CN']=0; Y[Y=='AD']=2; Y[Y=='MCI']=1; 
...
for train_index, test_index in skf.split(D,pd.to_numeric(Y.values)):
    ...
    Y = DTest['Diagnosis']
    idx=Y!='MCI'; Y = Y[idx]; Y[Y=='CN']=0; Y[Y=='AD']=1;
                       
    S = SubjTest1[0]['Stages'];  S=S.values[idx];
    auc1.append(metrics.roc_auc_score(pd.to_numeric(Y.values),S))
    S = SubjTest2[0]['Stages'];  S=S.values[idx];
    ...
    idxt=YT!='MCI'; YT = YT[idxt]; YT[YT=='CN']=0; YT[YT=='AD']=1;
